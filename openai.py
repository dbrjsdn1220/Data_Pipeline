...
def transform_extract_keywords(text):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "아래의 뉴스 본문에서 주요 키워드를 5개 추출해 주세요. 키워드는 콤마(,)로 구분된 한 줄 문자열로만 답변하세요. (예: 경제, 시장, 금리, 주식, 소비자)"}, # 해당 위치에서 키워드 5개를 추출할 수 있도록 프롬프트 작성성
...
def transform_classify_category(content):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": """다음 뉴스 본문을 읽고 가장 적절한 카테고리를 하나 선택하세요.
